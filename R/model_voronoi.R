...
model_voronoihull<- function(x){ # EC.params, EC.env){
  ...
  model_accuracy_voronoihull <- c(model_options_biomod$biomod_eval_method,
                             model_options_biomod$dismo_eval_method)

  # voronoihull algorithm does not need pseudo-absence points
  pa_number_point <- 0
  pa_number_point = 0
  if (pa_ratio > 0) {
  ...
  absen <- coord[c(which(model_compute_sdm@data.species == 0 |
                           is.na(model_compute_sdm@data.species))), names(coord)]
