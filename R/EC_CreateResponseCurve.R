...
EC_CreateResponseCurve <- function(out.model, model.name, species_algo_str) {
  ...
  if (!(length(model.values)==0)) {
    ...
    for (i in 0:((ncol(mean.values)-1)/curvesPerPage)) {
      png(file=file.path(EC.env$outputdir,sprintf("response_curve_%s_p%d.png",
      ...
      for (j in ((i*curvesPerPage + 1):min((i+1)*curvesPerPage, ncol(mean.values)))) {
        range.values = seq(min(model.values[,j], na.rm=TRUE),
                           max(model.values[,j], na.rm=TRUE), length.out=100)
        temp.data = mean.values
        temp.data[,j] = range.values
        if (model.name == "brt") {
          colnames(temp.data) = env.vars
          new.predictions = predict(out.model, as.data.frame(temp.data),
        ...
        df1 <- data.frame(range.values, new.predictions)  # save the response curve for later use
