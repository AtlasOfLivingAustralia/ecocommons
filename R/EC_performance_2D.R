...
EC_performance_2D <- function(obs,  # presence/absence (binary)
  ...
  best <- list(
    ...
    pred    = temp$tpv[pick1min(temp$L.pred == min(temp$L.pred, na.rm = TRUE))],
    all     = temp$tpv[pick1min(temp$L.all == min(temp$L.all, na.rm = TRUE))],
    eq.diag = temp$tpv[pick1min(temp$L.eq.diag == min(temp$L.eq.diag, na.rm = TRUE))]
  )
  ...
  for (v in names(best)) { # v<-"eq.diag"
    ...
    rangeperf[dimnames(rangeperf)[[1]] == v, 1:2] <- c(min(the.range, na.rm = TRUE),
                                                     max(the.range, na.rm = TRUE))
  }
  ...
  # Rescale
  temp$L.eq.diag <- temp$L.eq.diag / max(temp$L.eq.diag, na.rm = TRUE)
  ...
  if (make.plot != "") {
    ...
    errs <- tidyr::pivot_longer (temp, names_to = "tpv", names_repair = "minimal",
                                 cols = c("tpr", "fpr", "tnr", "fnr", "fdr",
                                               "fors", "L.diag", "L.pred", "L.all",
                                               "L.eq.diag"))
    names(errs)[4] <- c("measure")
    ...
    selected_rows = c(pick1min(temp$L.diag),
                      pick1min(temp$L.pred == min(temp$L.pred, na.rm = TRUE)),
                      pick1min(temp$L.all == min(temp$L.all, na.rm = TRUE)),
                      pick1min(temp$L.eq.diag == min(temp$L.eq.diag, na.rm = TRUE)))
 ...
 # write.csv(temp, paste(EC.env$outputdir,species_algo_str,'_performance.csv'))
 # write.csv(eval.stats, paste(EC.env$outputdir,species_algo_str,'_evaluation_stats.csv'))
 # write.csv(loss.table, paste(EC.env$outputdir,species_algo_str,'_loss_summary.csv'))
