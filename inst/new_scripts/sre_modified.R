#######################################################################
###   EcoCommons script to Surface Range Envelope - using biomod2   ###
#######################################################################
##
...
##  This script runs Surface Range Envelope profile to define a 
##  multidimensional space bounded by the min and max values of env
##  variables for all occurrences as the species potential range. It
##  uses the biomod2 package on R to set up parameters, dismo functions, 
...
#===================================================================
## Load and edit your dataset with parameters specific to Surface Range Envelope
...
biomod.selected.models <- EC.params$selected_models #'all' when all models have to be used to render projections or a subset vector of modeling.output models computed (eg, = grep('RF', getModelsBuiltModels(myBiomodModelOut)))
...
#===================================================================
## Surface Range Envelope profile
...
if (all(enviro.data.type == 'continuous') && (!is.null(enviro.data.constraints) || enviro.data.generateCHull)) {
    model.proj <-
    ...
    EC_GRDtoGTIFF(file.path(getwd(),
                     ...
                     algorithm=ifelse(is.null(EC.params$subset), "sre", sprintf("sre_%s", EC.params$subset)),
