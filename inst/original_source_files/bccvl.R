...
# You can jump this step if runnning remotely at home
r <- getOption("repos")
...
necessary=c("ggplot2","tools", "rjson", "dismo","SDMTools", "gbm", "rgdal", "rgeos", "pROC", "png", "gstat", "biomod2", "gdalUtils", "spatial.tools", "ggdendro", "rmaxent","raster") #list the libraries needed
...
### Load the parameters ###
#file.edit('params.json')
params = rjson::fromJSON(file="runSDM-params-ann-example.json")
...
# Provide simple names for all the parameters
...
bccvl.species.read <- function(filename, month_filter=NULL) {
    if (!is.null(filename)) {
        ...
        # keep year column if exists
        col_kept = c("lon","lat")
        if ('year' %in% colnames(csvfile)) {
            col_kept = append(col_kept, 'year')
        }
...
# retrieve coordinate reference from both user and climate data
...
## Use BIOMOD package to format data
...
bccvl.biomod2.formatData <- function(true.absen=NULL,
    ...
    if (is.null(true.absen)) {
        # create an empty data frame for background points
...
#------------------------------------------------------------------------------------------------------------
generate_background_data = function(rastermask, bgsize)
{
...
generate_biased_background_data = function(bg, biasfile, bgsize)
{
  ## Apply bias 
  # If a bias file is selected, apply the pattern of bias in the occurrences to the background points.
  # NB. This is applied to the background records remaining after applying the constraints area selection.
  # The argument 'size' should be ignored, and all background records from the previous step should be used.

  ## Load bias file
  bias <- raster(biasfile)

  # Convert background points to spatial points formal class
  bg_sp_class<-SpatialPoints(coords=cbind(bg$lon,bg$lat), proj4string=CRS(as.character(NA)))

  # Take a random sample of the candidate background points, weighting
  # sampling probability by the bias grid's value at the points
...
generate_response_curve = function(model, predvars, species, oformat)
{
  # Overall response curves
  png(filename=file.path(bccvl.env$outputdir, paste0("response_curves_", species, "_maxent_", oformat, ".png")),
        width=800, height=800)
  response(model, at=mean, range='p', expand=5) 
  dev.off()

  # Save individual response curve
  for (varname in predvars) {
    png(filename=file.path(bccvl.env$outputdir, paste0("response_curve_", species, "_maxent_", oformat, "_", varname, ".png")),
        width=800, height=800)
...
# Load plot function for continuous predictor variables for the occurrence data
exploratoryPlots.contin.occ = function(dataToExplore, listVars, doCorrelation=TRUE, outerTitle="", fnamePrefix="occurrence_predictors_", outputdir="output") 
{
...
dev.off()
  ...
  # Correlations, if requested  #Comment from Chantal: we always want to do correlations, so can we get rid of 'doCorrelation' argument?
...
# Load plot function for categorical predictor variables for the occurrence data
exploratoryPlots.categ.occ = function(dataToExplore, listVars, outerTitle="", fnamePrefix="occurrence_predictors_", outputdir="output") {
...
# Load plot function for continuous predictor variables for the background data
exploratoryPlots.contin.bg = function(dataToExplore, listVars, doCorrelation=TRUE, outerTitle="", fnamePrefix="background_predictors_", outputdir="output") 
{
  nVars = length(listVars)
...
exploratoryPlots.categ.bg = function(dataToExplore, listVars, outerTitle="", fnamePrefix="background_predictors_", outputdir="output") {
...
#------------------------------------------------------------------------------------------------------------
## Evaluation statistics. Return equal sensitivity specificity threshold.
saveEvaluationStatistics = function(occur, absen, model, predictor, species)
{
    ...
    png(file=file.path(bccvl.env$outputdir, paste0("density_", species, "_maxent.png")),
        width=800, height=800)
    density(evaluation)
    dev.off()
    ...
    write.csv(threshold_matrix, file=file.path(bccvl.env$outputdir, paste0('evaluation_', species, '_maxent.csv')))
...
computeAreaExtentOccuopancy <- function(occur, species)
{
  writeLines('Compute Area Extent Occuopancy ...')
  # Add land area (note, this land is built in to the package, it is possible to add own shapefile if needed)
  data(land)

  # data must be set out as in columns labeled Latitude, Longitude, Species (in this order) and year if needed
  cnames = names(occur)
  ...
  if (!('year' %in% cnames)) {
    occur = occur[, c('lat','lon','species')]
      ...
      aoo_eoo <- IUCN.eval(occur, file_name=file.path(bccvl.env$outputdir, paste0("aoo_eoo_", species, ".csv")))
      aoo_eoo <- aoo_eoo[c('taxa', 'EOO', 'AOO', 'Nbe_unique_occ.')]
      colnames(aoo_eoo) <- c('taxa', 'EOO', 'AOO', 'Number of unique occurrences')
      write.csv(aoo_eoo, file=file.path(bccvl.env$outputdir, paste0("aoo_eoo_", species, "_maxent.csv")))

      # Run IUCN evaluation to generate map
      # The image is returned in a folder called "IUCN__results_map".
      # Save as aoo_eoo_species.genus.png
      ...
      # AOO and EOO evaluation per year
      occur = occur[, c('lat', 'lon', 'species', 'year')]
      ...
      if ((yrmax - yrmin) >= 10) {  
        ...
        # move ymin back to have 5-year step 
      ...
      # Combine the rows of the list together into a data frame
      aoo_eoo_year <- bind_rows(aoo_eoo_result)
      aoo_eoo_year['year'] = names(aoo_eoo_result)

      # Save as csv
      # NB. the csv should show the year (subset) in the first column and then add the 4 columns ID, EOO, AOO, Nbe_unique_occ
      aoo_eoo_year = aoo_eoo_year[c('year', 'EOO', 'AOO', 'Nbe_unique_occ.')]
      names(aoo_eoo_year) <- c('year', 'EOO', 'AOO', 'Number of unique occurrences')
      ofname = file.path(bccvl.env$outputdir, paste0('aoo_eoo_year_', species, '_maxtent.csv'))
      write.csv(aoo_eoo_year, ofname, row.names = TRUE)
      ...
      png(filename=file.path(bccvl.env$outputdir, paste0('aoo_year_', species, '_maxent.png')),
           width=800, height=800)
      ...
      png(filename=file.path(bccvl.env$outputdir, paste0('eoo_year_', species, '_maxent.png')),
           width=800, height=800)
...
#-------------------------------------------------------------------------------
## Threshold map based on equal sensitivity specificity threshold
...
# presence (red) >= threshold, absence (blue) < threshold
bccvl.generateThresholdMap <- function(proj_raster, threshold) {
    # return 1 for present, 0 for absence
    writeLines('Generating presence/absence map ...')
    present_map <- calc(proj_raster, fun=function(x) { return(as.integer(x >= threshold))})

    threshold_map = file.path(bccvl.env$outputdir, paste0("equal.sens.spec_", occur.species, "_maxent_cloglog.tif"))
    writeRaster(present_map, threshold_map, format="GTiff", options=c("COMPRESS=LZW", "TILED=YES"), overwrite=TRUE)
    
    threshold_map = file.path(bccvl.env$outputdir, paste0("equal.sens.spec_", occur.species, "_maxent_cloglog.png"))
